function GetServiceGuideChannels() {
...
function SaveChannels() {
    ...
    if (TypeId == "") {
        $("#TypeId").focus();
        //Swal.fire({ icon: "info", text: "Please Channel Type" });
        ShowAlert("SaveChannel1", "0");
        return false;
    }
    ...
    Swal.fire({
        icon: 'question',
        html: ShowAlert('ConfirmSaveChannel', '1') + "<br/>" + myName,
        ...
        if (result.isConfirmed) {
            ...
            iObject.QRCode = $("#QRCode").val();
            iObject.QRCodeAlt = $("#QRCodeAlt").val();            
    ...
    $.ajax(
        {
            ...
            success: function (response) {
                ...
                $("#QRCode").val(response.QRCode);
                $("#QRCodeAlt").val(response.QRCodeAlt);                
...
function Delete(recordId, Title) {
    ...
    Swal.fire({
        icon: 'question',
        html: ShowAlert('ConfirmDeleteChannel', '1') + "<br/> " + Title,
        //showCloseButton: true,
        showCancelButton: true,
        //focusConfirm: true,
        confirmButtonText: ShowAlert('YesButton', '1'),
